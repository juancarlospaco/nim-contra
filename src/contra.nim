...
template hardenedBuild*() =
  ...
  when defined(hardened) and defined(gcc) and not defined(objc) and not defined(js) and not defined(cpp):
    ...
    {.passC: hf, passL: hf.}
  when not defined(hardened) and defined(gcc) and not defined(objc) and not defined(js) and not defined(cpp):
    when defined(danger): {.passC: "-flto -ffast-math -march=native -mtune=native".}
